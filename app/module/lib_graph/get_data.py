...
def get_ok_data_before(days_delta):
    one_year_ago = now() - timedelta(days=days_delta)
    ...
    results = Event.objects.filter(
        ...
        |Q(status__icontains="Acompte OK")
        |Q(status__icontains="Post Presta")
    ).filter(signer_at__lt=one_year_ago
    ).select_related(
        'client_id',  # Suppose un ForeignKey vers AppClient
        'event_product_id',  # Suppose un ForeignKey vers AppEventProduct
        'event_details_id'  # Suppose un ForeignKey vers AppEventDetails
    ...
    # df['Date-Event'] = pd.to_datetime(df['Date-Event'], errors='coerce')
    # filtered_df = df[df['Date-Event'].dt.year > 2023]
    # print(filtered_df)
