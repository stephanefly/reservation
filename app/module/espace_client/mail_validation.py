...
def send_mail_validation(event):
    ...
    with open(os.path.join(MAIL_TEMPLATE_REPOSITORY, "mail_validation.html"), 'r', encoding='utf-8') as fichier_html:
        html_message = fichier_html.read()
    soup = BeautifulSoup(html_message, 'html.parser')

    soup_completed = complete_mail_validation(event, soup)
...
def complete_mail_validation(event, soup):
    ...
    soup.find('b', class_='prestation').string = selected_booths
