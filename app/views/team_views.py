...
def _team_event_queryset():
    return Event.objects.select_related(*TEAM_EVENT_RELATIONS)
...
def template_to_do(request):
    ...
    lst_event_prio = _team_event_queryset().filter(
...
def team_post_presta(request):
    lst_post_event = _team_event_queryset().filter(
        signer_at__isnull=False,
        status__in=['Post Presta']
    ).prefetch_related('event_team_members').order_by('event_details__date_evenement')
...
def relance_client(request):
    event_to_relance = _team_event_queryset().filter(
        status__in=["Prolongation", "Last Chance", "Last Rappel", "Initied"]
    ).prefetch_related(
        Prefetch(
            'relances',
            queryset=EventRelance.objects.order_by('-date_relance'),
            to_attr='ordered_relances',
        )
    ).order_by("event_details__date_evenement")
...
def info_relance_client(request, event_id):
    event = get_object_or_404(_team_event_queryset(), pk=event_id)
    lst_relance_event = EventRelance.objects.filter(event=event).order_by('date_relance')
    event_reduc_total = (event.reduc_all or 0) + (event.reduc_product or 0)
